...
function Assert-PhaseOneTestInventory {
    ...
    if ($backendText -notmatch '(?m)^251 tests collected in ') {
        throw "Backend test inventory is not exactly 251 collected tests."
    }
    ...
    Write-Host "Test inventory verified: backend=251, frontend=43, e2e=2"
}
