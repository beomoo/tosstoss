...
function Assert-PhaseTwoCP3C2B2BTestInventory {
    ...
    if ($backendText -notmatch '(?m)^659 tests collected in ') {
        throw "Backend test inventory is not exactly 659 collected tests."
    }
    ...
    Write-Host "Test inventory verified: backend=659, frontend=43, e2e=2"
}
